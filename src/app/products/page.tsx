...
export default async function ProductsPage() {
  ...
  try {
    products = await prisma.product.findMany({ include: { category: true, images: { take: 1, orderBy: { order: "asc" } } }, orderBy: [{ order: "asc" }, { name: "asc" }] });
  } catch {
    // Keep the route available while a local database tunnel is offline.
  }
