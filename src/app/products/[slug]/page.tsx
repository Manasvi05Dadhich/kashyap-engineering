...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    product = await prisma.product.findUnique({ where: { slug }, include: { category: true, images: { orderBy: { order: "asc" } } } });
  } catch {
    // Treat an unavailable database like a missing product instead of throwing a server error.
  }
  if (!product) notFound();
