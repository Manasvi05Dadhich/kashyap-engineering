...
type CategoryWithProducts = Prisma.CategoryGetPayload<{
  include: { products: { include: { images: { take: 1; orderBy: { order: "asc" } } } } };
}>;
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    category = await prisma.category.findUnique({ where: { slug }, include: { products: { include: { images: { take: 1, orderBy: { order: "asc" } } }, orderBy: { order: "asc" } } } });
  } catch {
    // Treat an unavailable database like a missing category instead of throwing a server error.
  }
